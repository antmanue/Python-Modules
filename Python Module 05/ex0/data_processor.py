...
class LogProcessor(DataProcessor):
    def validate(self, data: typing.Any) -> bool:
        if isinstance(data, dict) and "log_level" in data and "log_message" in data:
            return True
        elif data != ([]) and isinstance(data, list):
            for item in data:
                if not isinstance(item, dict) or "log_level" not in item or "log_message" not in item:
                    return False
...
def main() -> None:
    ...
    print(f"Trying to validate input 'Hello': {log_process.validate('Hello')} ")
