...
def main() -> None:
    ...
    p_capt_og = [item for item in players if item[0].isupper() == True]

    score_dict = {player: random.randint(0, 1000) for player in p_all_capitalized}

    score_avg = round((sum(score_dict.values()) / len(score_dict.keys())), 2)

    high_score_dict = {player: score for player, score in score_dict.items() if score > score_avg}
